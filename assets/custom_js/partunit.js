jQuery('document').ready(function () {
    ...
    table.dataTable({
        ...
        "bProcessing": false,
        "serverSide": true,
        "sAjaxSource": BASEURL + "WEB/partunit/getpartunit",
        "sServerMethod": "POST",
        "aoColumns": [
            ...
            {"data": "id", "bSearchable": false, "bSortable": false, "mRender": function (data, type, full) {
                   return '<a href="' + BASEURL + 'partunitdetail/' + btoa(data) + '" class="btn btn blue" data-bind="' + data + '"><i class="fa fa-pencil-square-o" aria-hidden="true"></i></a><a href="javascript:;" class="btn btn red remove_category" data-bind="' + data + '"><i class="fa fa-trash-o" aria-hidden="true"></i></a>';
               }}
        ],
    ...
    //Add Product
    var add_restaurant = $("#add_category"),
    ...
    add_restaurant.validate({
        ...
        messages: {
            unit_name: {
                required: "Please Enter Part Unit Name"
            }
        },
        rules: {
            unit_name: {
        ...
        submitHandler: function (add_restaurant) {
            ...
            $.ajax({
                url: BASEURL + 'WEB/partunit/insert_partunit',
                type: 'POST',
                data: formData,
                async: true,
                dataType: 'json',
    ...
    //Edit restaurant
    var add_restaurant = $("#edit_chapter"),
            r = $(".alert-danger", add_restaurant),
            i = $(".alert-success", add_restaurant);
    ...
    add_restaurant.validate({
        ...
        submitHandler: function (add_restaurant) {
            i.show(), r.hide()
            //submit data with ajax 
            var formData = new FormData($("#edit_chapter")[0]);
            $.ajax({
                url: BASEURL + 'WEB/partunit/edit_partunit',
                type: 'POST',
                data: formData,
                async: true,
                dataType: 'json',
    ...
    $(document).on("click", ".remove_category", function () {
        ...
        bootbox.confirm("Are you sure do you want to delete this unit ?", function (result) {
            ...
            if (result) {
                ...
                //define ajax path
                var path = BASEURL + 'WEB/partunit/deletepartunit';
                $.post(path, {
                    data: dataString
                },
                function (res) {
