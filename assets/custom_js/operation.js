jQuery('document').ready(function () {
    ...
    table.dataTable({
        ...
        "bProcessing": false,
        "serverSide": true,
        "sAjaxSource": BASEURL + "WEB/operation/geltabledata",
        "sServerMethod": "POST",
        "aoColumns": [
            {"data": "no", "bSearchable": false, "bSortable": false, "bVisible": true},
            {"data": "name", "bSearchable": true, "bSortable": true},
            {"data": "url", "bSearchable": true, "bSortable": true},
            {"data": "id", "bSearchable": false, "bSortable": false, "mRender": function (data, type, full) {
                   return '<a href="' + BASEURL + 'operationdetail/' + btoa(data) + '" class="btn btn blue" data-bind="' + data + '"><i class="fa fa-pencil-square-o" aria-hidden="true"></i></a><a href="javascript:;" class="btn btn red remove_item" data-bind="' + data + '"><i class="fa fa-trash-o" aria-hidden="true"></i></a>';
               }}
        ],
    ...
    add_item_var.validate({
        ...
        messages: {
            name: {
                required: "Please Enter Operation Name"
            },
            url: {
        ...
        submitHandler: function (add_restaurant) {
            ...
            $.ajax({
                url: BASEURL + 'WEB/operation/insert_data',
                type: 'POST',
                data: formData,
                async: true,
                dataType: 'json',
    ...
    edit_item_var.validate({
        ...
        submitHandler: function (add_restaurant) {
            ...
            $.ajax({
                url: BASEURL + 'WEB/operation/edit_data',
                type: 'POST',
                data: formData,
                async: true,
                dataType: 'json',
                success: function (data) {
                    if (data.success)
                    {
                        toastr.success(data.message, 'Success');
                         setTimeout(function(){ 
                            window.location.href = BASEURL+'operation';
                        }, 3000);
                    } 
                    else
                    {
    ...
    $(document).on("click", ".remove_item", function () {
        ...
        bootbox.confirm("Are you sure do you want to delete this unit ?", function (result) {
            ...
            if (result) {
                ...
                //define ajax path
                var path = BASEURL + 'WEB/operation/deletedata';
                $.post(path, {
                    data: dataString
                },
                function (res) {
