...
if (!function_exists('page_view')) {
    ...
    function page_view() {
        ...
        //if (isset($CI->session->userdata['site']) && !empty($CI->session->userdata['site']) && $CI->session->userdata['site'] == 'toughglass') {
            //_pre(uri_string());
            $isPermission = false;
            $allMenu = $CI->session->prevision_data;
            if(!empty($allMenu)){
                foreach ($allMenu as $item){
                //_pre($item['pv_url']);
                //_pre(uri_string());
                    if(strcmp($item['pv_url'],uri_string())==0){
                        $isPermission = true;
                        //_pre("YEs" . $isPermission . " : ".uri_string()." : ".$item['pv_url']." : ".strcmp($item['pv_url'],uri_string()));
                    }
