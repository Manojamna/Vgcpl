...
class Adminlogin extends My_Base_Controller {

    public function __construct() {
        parent::__construct();
        $this->load->model('login_model');
        
        if(isset($this->session->userdata['user_data']['user_id']) && $this->session->userdata['user_data']['user_id'] && $this->router->method == 'index')
    ...
    public function index() {
        ...
        //$data = array('page_title' => 'VGCPL | Admin Login', 'login_view' => true, 'error' => $this->session->flashdata('error'));
        $this->load->view(backend_view() . '/profile/login');
    }
    ...
    public function login_check() {
        ...
        if (!empty($email) && !empty($password)) {
            ...
            $authenticate = $this->login_model->authenticate($email, $password);
            ...
            if ($authenticate) {
                ...
                $session_data['is_login'] = 1;
                $session_data['site'] = 'vgcpl';
                $session_data['user_data'] = $authenticate;
                //_pre($authenticate);
    ...
    public function adminprofile($id = '') {
        ...
        if (!empty($id)) {
            $data['records'] = $this->login_model->Getprofile($id);

            if (!empty($data['records'])) {
                $data['u_id'] = $id;
                $this->template->view(backend_view() . 'profile/adminprofile', $data);
            } else {
    ...
    public function update_profile() {
        ...
        $fields = array('user_f_name','user_l_name', 'mobile','email','user_bio','address','username');
        ...
        if (isset($_FILES['image']['name']) && !empty($_FILES['image']['name'])) {
            ...
            if (isset($old_image) && !empty($old_image)) {
                $is_exist = AUTHOR_IMAGE_PATH.$old_image;
                if(file_exists($is_exist))
                {
                    unlink(AUTHOR_IMAGE_URL . $old_image);
                }
            }
            ...
            $profile_data['profile_pic'] = $file_name;
        }
        $status = $this->login_model->Editprofile($profile_data, $u_id);
        if ($status == TRUE) {
            //Set updated data to session
            $getdata = $this->login_model->Getprofile($u_id);

            $session_data['is_login'] = 1;
            $session_data['user_data'] = $getdata;
            $this->session->set_userdata($session_data);
    ...
    public function change_password() {
        ...
        $password_info = $this->login_model->current_password_check($current_password, $session_data['id'], $session_data['email']);
        //_pre($password_info);
        if (isset($password_info) && !empty($password_info)) {
            $user_new_data = array('password' => md5($new_password));
            $user_id = $session_data['id'];
            $this->login_model->update_password($user_new_data,$user_id);
            return TRUE;
            //$return['success'] = true;
        } else {
            echo "error";
    ...
    public function forget_password() {
        ...
        $email = $this->input->post('email');
        $result = $this->login_model->check_mail($email);
        $return = array();
        if ($result != '') {
            $data['name'] = $result->fname . ' ' . $result->lname;
            $data['coded_email'] = base64_encode($email);
            $data['token'] = base64_encode(rand(100, 999));
            $result = $this->login_model->updated_token($data['token'], $email);
    ...
    public function reset_password() {
        ...
        $status = $this->login_model->check_token($data['token']);
        if ($status == TRUE) {
    ...
    public function resetpassword_process() {
        ...
        $new_password = $this->input->post('new_password');
        $return = array();
        $status = $this->login_model->passwordreset($q1, $token, $new_password);
        if ($status) {
            $return['success'] = true;
        }
        else
    ...
    private function send_mail($email, $message) {
        $this->email->initialize(array(
            'protocol' => 'smtp',
            'smtp_host' => 'mail.moontechnolabs.com',
